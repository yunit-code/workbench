<template>
  ...
  >
    <div
      class="i-workbench-card-item"
      v-for="(item, index) in listData"
      :key="`workbench-card-${index}`"
    >
      <div class="card-item-ad" :class="{ publish: item.publish !== '0' }">
        <span class="card-item-tip" v-if="item.isTip">
          <a-icon type="bell" />内容有更新,请重新发布
        </span>
      </div>
        ...
        <div class="des-handle">
          <a-dropdown>
            ...
            <template slot="overlay">
              <a-menu
                @click="dropdownClick"
                v-if="propData.buttonList && propData.buttonList.length > 0"
              >
                <template v-for="(bitem, bindex) in propData.buttonList">
                  <a-menu-item
                    v-if="bitem.buttonName"
                    :key="bindex"
                    @click="clickButtonHandle(item, bitem)"
                  >
                    {{ bitem.buttonName }}</a-menu-item
                  >
                </template>
              </a-menu>
            </template>
  ...
  methods: {
    ...
    convertAttrToStyleObject() {
      ...
      if (this.propData.cardBgSize && this.propData.cardBgSize == "custom") {
        cardStyleObject["background-size"] =
          (this.propData.cardBgSizeWidth
            ? this.propData.cardBgSizeWidth.inputVal +
              this.propData.cardBgSizeWidth.selectVal
            : "auto") +
          " " +
          (this.propData.cardBgSizeHeight
            ? this.propData.cardBgSizeHeight.inputVal +
              this.propData.cardBgSizeHeight.selectVal
            : "auto");
      ...
      if (this.propData.cardPositionX && this.propData.cardPositionX.inputVal) {
        cardStyleObject["background-position-x"] =
          this.propData.cardPositionX.inputVal +
          this.propData.cardPositionX.selectVal;
      }

      if (this.propData.positionY && this.propData.positionY.inputVal) {
        cardStyleObject["background-position-y"] =
          this.propData.positionY.inputVal + this.propData.positionY.selectVal;
      }
      if (this.propData.cardPositionY && this.propData.cardPositionY.inputVal) {
        cardStyleObject["background-position-y"] =
          this.propData.cardPositionY.inputVal +
          this.propData.cardPositionY.selectVal;
      }
        ...
        if (this.propData.hasOwnProperty.call(this.propData, key)) {
          ...
          switch (key) {
            ...
            case "cardFont":
              ...
              cardStyleObject["font-weight"] =
                element.fontWeight && element.fontWeight.split(" ")[0];
              cardStyleObject["font-style"] = element.fontStyle;
              cardStyleObject["font-size"] =
                element.fontSize + element.fontSizeUnit;
              cardStyleObject["line-height"] =
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .i-workbench-card-item",
        cardStyleObject
      );
      // this.initData();
    },
