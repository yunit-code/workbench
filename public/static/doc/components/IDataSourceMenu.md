...
IDataSourceMenu
## 组件类ID（classId）
idm.componet.workbench.dataSourceMenu
## 组件开发语言（comLangue）
vue
## 组件类型（comType）
common
...
#### 菜单图标【menuImgUrl】
菜单图标，可上传
#### 默认选中【defaultSelectKey】
设置默认选中项，若设置多个，则只有第一个有效
### 样式设置
...
#### 菜单选中自定义函数【customSelectFunction】
当菜单选项切换时触发，参数为：
``` json
{	
	"customParam":"自定义的参数",
	"selectedItem":"选中项的原始数据"
}
```